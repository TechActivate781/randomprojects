...
void processPlus(int whomad, int cards[100][2], int amount, int amounts[2]){ 
    if(whomad == 1){
        printf("You've been given %d extra cards as the computer used a Plus . They are:\n", amount, amount);
...
int chooseCompTurn(int current[2], int cards[100][2], int amount){ //moving this down because for some reason the repeating functions cmoe here.
    ...
    // if there are 2 cards that you cna play, do that
    ...
    // IDEA: make it a whole array instead. this could also be held in main() so i can add/remove? nah. gonna be difficult changing it :/
    ...
    for(int i = 0; i < amount; i++){
        switch(cards[i][0]){
            //rewriting to a switch since i believe they're faster??
            case 0:
            numOfCards[0]++;
            break;
            ...
            numOfCards[14]++;
            break;
            // not bothering with a default since it shouldn't be anything other than these :^)
        }

        // time to do the same but for color.
    ...
    for(int i = 0; i < amount; i++){
        if(cards[i][0] == 12 || cards[i][0] == 13){
            
        }
    }

    if(current[0] == 13 || current[0] == 12 && amount > 1){
