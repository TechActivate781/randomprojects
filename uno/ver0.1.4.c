...
int chooseCompTurn(int current[2], int cards[100][2], int amount){
    ...
    int numPlus; // there's 100% a better way to do it but idk it so gonna do it MY WAY :3
    int numOne;
    int numTwo;
    int numThree;
    int numFour;
    ...
    int numNine;
    int numZero;
    // please help me this is so NOT HOW YOU DO IT
    int numSkip; // this will include number of reverse since they're practically the same
    int numChange;
    ...
    if(current[0] == 13 || current[0] == 12 && numPlus > 1){
